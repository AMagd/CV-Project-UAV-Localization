#include <Arduino.h>
// [431,  41, 218, 768,  93, 629, 925, 212, 421, 155]
int ID1 = 1023; // Decimal representation of the ID of the first LED
int ID2 = 1023; // Decimal representation of the ID of the Second LED
...
int maxFPS = 50; // maximum FPS of the camera
int CameraFPS; // = 30; // used such that every frame bit will be displayed in one frame
...
{
  potVal = analogRead(0);
  CameraFPS = map(potVal, 0, 1023, 90, maxFPS); // used such that every frame bit will be displayed in one frame
  delayTime = (1.0/CameraFPS)*1000; // delay time between each bit in ms
  Serial.println(CameraFPS);
  
  for (int i=0; i<10; i++){
    digitalWrite(8, binaryID1[i]);
    digitalWrite(9, binaryID2[i]);
    digitalWrite(10, binaryID3[i]);
